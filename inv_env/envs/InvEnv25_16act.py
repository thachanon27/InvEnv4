...
#         print("on_hand1 =", on_hand1)
        ...
        s_penal = 8
        if overage1 < 500:
            extra_penalty1 = s_penal*1000000*1.5
        if overage2 < 500:
            extra_penalty2 = s_penal*1000000*1.5
        if overage3 < 500:
            extra_penalty3 = s_penal*1000000*1.5
            
        penal = 15
        if overage1 <= 0:
            extra_penalty1 = penal*1000000*4  # ถ้า < 4500 แต่ ไม่ < 0 ตรงนี้จะข้ามไป ไม่โดน penalty แต่ < 0 ด้วย 5 ล้านจะถูกแทนด้วยค่า 9 ล้าน
        if overage2 <= 0:
            extra_penalty2 = penal*1000000*4
        if overage3 <= 0:
            extra_penalty3 = penal*1000000*6

        if overage1 > 7000:
            extra_penalty1 = penal*1000000*3
        if overage2 > 6500:
            extra_penalty2 = penal*1000000*3
        if overage3  > 6000:
            extra_penalty3 = penal*1000000*3
...
#                                        extra_p_on1_1 + extra_p_on1_2 + extra_p_on1_3 + extra_p_on2_1 + extra_p_on2_2 + extra_p_on2_3)) / 1000000)) / 500
        ...
        # ใส่ _ = ยังไม่เอามาคิด ถ้าจะคิดก็เอา _ ออก    #450
        reward = (550 + (sales_revenue)/1000000 + extra_reward1/1000000 - ((purchase_cost + holding + penalty_lost_sale
                            ...
                            + sum_extra_penalty + sum_extra_penalty_2 + sum_extra_penalty_3
                            + (
                                       extra_p_on1_1 + extra_p_on1_2 + extra_p_on1_3 + extra_p_on2_1 + extra_p_on2_2 + extra_p_on2_3)) / 1000000)) / 550   #650
